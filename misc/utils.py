...
def soft_IOULoss(pred, gt):
    B = pred.size(0)
    gt = gt.detach()
    TP = pred * gt
    merge = torch.clamp(gt+pred, min=0, max=1)
    TP = torch.sum(TP, dims=(1, 2, 3))
    merge = torch.sum(merge, dims=(1, 2, 3)) + torch.ones_like(TP).cuda() * 1e-16
    
    return -(TP / merge).mean()
...
def adjust_learning_rate(optimizer, base_lr1,base_lr2, max_iters,
    ...
    # optimizer.param_groups[3]['lr'] = lr2
    return lr1,lr2
